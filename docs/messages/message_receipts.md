...
For marking channels as read/unread and retrieving unread counts, see [Unread Counts](/chat/docs/python/unread/).
...
client.update_channel_type("targetChannelType", delivery_events=True)
```
...
# When updating an existing channel type
client.update_channel_type("targetChannelType", read_events=True)
```
...
For handling these events and updating unread counts, see [Unread Counts](/chat/docs/python/unread/).
