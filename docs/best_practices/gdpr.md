...
GDPR gives EU citizens the right to request access to their information and the right to have access to this information in a portable format. Stream Chat covers this requirement with the [Export User](/chat/docs/python/exporting_channels/) method.

This method can only be used with server-side authentication.

```python
response = server_client.export_user(user_id);
```
...
- [Deleting a batch of users](/chat/docs/python/update_users/#deleting-many-users/)

- [Deleting a batch of channels](/chat/docs/python/channel_delete/#deleting-many-channels/)
