...
> [!NOTE]
> In legacy permission system users can never access users nor channels from other teams. In [Permissions V2](/chat/docs/python/chat_permission_policies/) it is possible to alter this behavior using multi-tenant permissions.
...
In tables below you will find default permission grants for builtin roles that designed for multi-tenant applications. They are useful for [multi-tenant applications](/chat/docs/python/multi_tenant_chat/) only.
...
response = client.query_team_usage_stats(month="2024-01")
...
if response.get("next"):
    next_page = client.query_team_usage_stats(limit=10, next=response["next"])
```
