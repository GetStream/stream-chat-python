You can delete or truncate a channel to remove its contents. To remove only messages while preserving the channel, see [Truncate Channel](/chat/docs/python/truncate_channel/).
...
The  `deleteChannels`  response contain a taskID which can be polled using the [getTask endpoint](/chat/docs/python#tasks-gettask) to check the status of the deletions.
