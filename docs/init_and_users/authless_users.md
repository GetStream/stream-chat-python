...
Guest users have a limited set of permissions. You can read more about how to configure permissions [here](/chat/docs/python/chat_permission_policies/). You can create a guest user session by using `setGuestUser` instead of `connectUser`.

You can generate a guest user in a front end client by using the following code:


> [!NOTE]
> The user object schema is the same as the one described in the [Initialization and Users](/chat/docs/python/init_and_users/) portion of the docs.
