...
If you intend to make any change to the message, you should return a JSON encoded response with the same message structure. Please note that not all message fields can be changed, the full list of fields that can be modified is available in the [rewriting messages](/chat/docs/python/before_message_send_webhook/#rewriting-messages/) section.
