...
- Your app has push notifications configured for at least one provider [in your app settings](/chat/docs/python/app_setting_overview/)

- You have a user that has at least one [device](/chat/docs/python/#push_devices/) associated
...
client.check_push(
  {
    ...
    "skip_devices": True,
    "user_id": user["id"],
    "apn_template": "{\"aps\":{\"alert\":{\"title\":\"{{ sender.name }} @ {{ channel.name }}\",\"body\":\"testing out new stuff:{{ message.text }}\"},\"category\":\"NEW_MESSAGE_2\"}}",
  }
)
```
